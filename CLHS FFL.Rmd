...
ReactiveWinLowerQuantile <- reactive({
  LongScoringData %>%
...
ReactiveWinUpperQuantile <- reactive({
  LongScoringData %>%
    filter(WeekType == "Regular" & seasonId %in% input$YearInput) %>%
    arrange(seasonId ,matchupPeriodId) %>% 
    group_by(firstName) %>% 
...
renderValueBox(
  valueBox(ReactiveLongestWinStreak()
           ,color = ifelse(ReactiveLongestWinStreak() < ReactiveWinLowerQuantile()  ,"danger" ,ifelse(ReactiveLongestWinStreak() <= ReactiveWinUpperQuantile()  ,"warning" ,"success")))
)
...
ReactiveLoseLowerQuantile <- reactive({
  LongScoringData %>%
    mutate(Lose = if_else(Win == 1 ,0 ,1)) %>%
    filter(WeekType == "Regular" & seasonId %in% input$YearInput) %>%
    arrange(seasonId ,matchupPeriodId) %>% 
...
ReactiveLoseUpperQuantile <- reactive({
  LongScoringData %>%
    mutate(Lose = if_else(Win == 1 ,0 ,1)) %>%
    filter(WeekType == "Regular" & seasonId %in% input$YearInput) %>%
    arrange(seasonId ,matchupPeriodId) %>% 
...
renderValueBox(
  valueBox(ReactiveLongestLoseStreak()
           ,color = ifelse(ReactiveLongestLoseStreak() > ReactiveLoseUpperQuantile()  ,"danger" ,ifelse(ReactiveLongestLoseStreak() >= ReactiveLoseLowerQuantile()  ,"warning" ,"success")))
)
